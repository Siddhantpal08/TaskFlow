...
import {
    ...
    Alert, Image, TextInput, ActivityIndicator, RefreshControl, KeyboardAvoidingView, Platform
} from 'react-native';
...
export default function ProfileScreen() {
    ...
    const { tasks, refreshAll } = useData();
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res?.success) {
                // updateUser already calls setUser — reset local state from updated user
                setEditing(false);
            } else {
    ...
    const handlePasswordChange = async () => {
        if (!oldPw || !newPw || !confirmPw) return Alert.alert('Error', 'Please fill in all fields.');
    ...
    return (
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <ScrollView style={s.container} contentContainerStyle={{ padding: 20, paddingBottom: 60 }}
                ...
                {/* Change Password */}
                <View style={s.section}>
                    <TouchableOpacity onPress={() => setShowPwChange(o => !o)} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Text style={s.sectionTitle}>Change Password</Text>
                        <Text style={{ color: t.accent, fontSize: 13 }}>{showPwChange ? '▲ Hide' : '▼ Show'}</Text>
                    </TouchableOpacity>
                    {showPwChange && (
                        <View style={{ marginTop: 12 }}>
                            <Text style={s.lbl}>Current Password</Text>
                            <TextInput style={s.inp} value={oldPw} onChangeText={setOldPw} secureTextEntry placeholder="••••••••" placeholderTextColor={t.t3} />
                            <Text style={s.lbl}>New Password</Text>
                            <TextInput style={s.inp} value={newPw} onChangeText={setNewPw} secureTextEntry placeholder="Min 6 characters" placeholderTextColor={t.t3} />
                            <Text style={s.lbl}>Confirm New Password</Text>
                            <TextInput style={s.inp} value={confirmPw} onChangeText={setConfirmPw} secureTextEntry placeholder="Repeat new password" placeholderTextColor={t.t3} />
                            <TouchableOpacity onPress={handlePasswordChange} disabled={pwSaving} style={[s.editBtn, { marginTop: 8, width: '100%', alignSelf: 'stretch' }]}>
                                {pwSaving ? <ActivityIndicator color="#000" size="small" /> : <Text style={{ color: '#000', fontWeight: '800', fontSize: 13 }}>Update Password</Text>}
                            </TouchableOpacity>
                        </View>
                    )}
                </View>
...
const s = StyleSheet.create({
    ...
    sectionTitle: { fontSize: 14, fontWeight: '800', color: t.t1, marginBottom: 8, letterSpacing: -0.3 },
    ...
    lbl: { fontSize: 12, color: t.t2, fontWeight: '700', marginBottom: 5, marginTop: 8 },
    inp: { backgroundColor: t.surf, borderWidth: 1, borderColor: t.border, borderRadius: 10, padding: 12, color: t.t1, fontSize: 14 },

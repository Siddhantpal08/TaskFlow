...
export default function Dashboard({ t, setPage, setTask }) {
    ...
    return (
        <div id="dashboard-main-view" style={{ padding: isMobile ? "12px 14px" : "20px 24px", display: "flex", flexDirection: "column", gap: 14, width: "100%", height: "100%", boxSizing: "border-box", overflow: "hidden" }}
            className="dash-root">
            <style>{`
                @keyframes statPulse {
            ...
            <div id="dash-hero" style={{
                ...
                border: `1px solid ${hero.color}28`, borderRadius: 14, padding: isMobile ? "12px 16px" : "14px 20px",
                display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", flexShrink: 0,
                gap: 12, flexWrap: "wrap",
            }}>
                <div style={{ flex: 1, minWidth: 0, width: isMobile ? "100%" : "auto" }}>
                    <div style={{ fontSize: isMobile ? 18 : 22, fontWeight: 800, color: t.t1, letterSpacing: "-0.4px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        {greeting}, {firstName} 👋
                        {!isMobile && (
                    ...
                    <div style={{ fontSize: isMobile ? 12 : 13, color: hero.color, marginTop: 4, fontWeight: 600 }}>
                        {hero.msg}
                    </div>
                </div>
            ...
            {/* ── Stats ── */}
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(3, 1fr)" : "repeat(4,1fr)", gap: 10, flexShrink: 0 }} className="stats-grid">
                <Stat val={total} label={isMobile ? "Total" : "Total Tasks"} note="all time" color={t.accent} onClick={() => setPage("tasks")}
            ...
            {/* ── Main grid ── */}
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 280px", gap: 14, flex: 1, minHeight: 0 }} className="dash-main-grid">
                ...
                <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 14, boxShadow: t.shadow, display: "flex", flexDirection: "column", minHeight: 0, overflow: "hidden" }}>
                    ...
                    {/* On mobile, insert Focus Today here if it exists */}
                    {isMobile && todayTasks.length > 0 && (
                        <div style={{ borderBottom: `1px solid ${t.border}`, background: "#F59E0B0A", flexShrink: 0 }}>
                            <div style={{ padding: "8px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <span style={{ fontSize: 12, fontWeight: 700, color: "#F59E0B" }}>⚡ Focus Today</span>
                            ...
                            <div style={{ display: "flex", overflowX: "auto", padding: "0 14px 10px", gap: 8 }}>
                                {todayTasks.map(tk => (
                                    <div key={tk.id} onClick={() => setTask(tk)} style={{ background: t.bg, border: `1px solid ${t.border}`, padding: "8px 12px", borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, minWidth: 200, flexShrink: 0 }}>
                    ...
                    <div style={{ padding: "10px 14px", borderBottom: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, flexWrap: "wrap", gap: 6 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center", overflowX: "auto", paddingBottom: 2 }}>
                            ...
                            {!isMobile && (
                                <button onClick={() => setPage("tasks")} style={{ background: "none", border: "none", cursor: "pointer", color: t.accent, fontSize: 12, fontWeight: 600, fontFamily: t.disp, display: "flex", alignItems: "center", gap: 3, marginLeft: 4, flexShrink: 0 }}>
                    ...
                    <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                        {filteredTasks.length === 0 ? (
                            <div style={{ padding: "12px 18px 24px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", height: "100%", minHeight: "200px" }}>
            ...
            {/* Mobile Accordion for Upcoming Events */}
            {isMobile && (
                <MobileAccordion t={t} title="Upcoming Events" icon={IC.cal}>
                    {events.length === 0 ? (
                        <EmptyState t={t} icon="cal" title="No upcoming events" description="Your schedule is clear." />
                    ) : events.slice(0, 4).map(ev => {
                        const d = new Date(ev.event_date);
                        const colors = [t.red, t.accent, t.green, t.amber];
                        const c = colors[ev.id % colors.length];
                        return (
                            <div key={ev.id} style={{ padding: "9px 0", borderBottom: `1px solid ${t.border}`, display: "flex", gap: 10, alignItems: "center" }}>
                                <div style={{ width: 34, borderRadius: 8, padding: "3px 0", textAlign: "center", background: c + "14", border: `1px solid ${c}28`, flexShrink: 0 }}>
                                    <div style={{ fontSize: 7.5, fontWeight: 700, color: c, fontFamily: t.mono, textTransform: "uppercase" }}>{d.toLocaleDateString("en-US", { month: "short" })}</div>
                                    <div style={{ fontSize: 15, fontWeight: 900, color: c, lineHeight: 1 }}>{d.getDate()}</div>
                                </div>
                    ...
                    {events.length > 0 && (
                        <button onClick={() => setPage("calendar")} style={{ width: "100%", marginTop: 8, background: "none", border: "none", cursor: "pointer", color: t.accent, fontSize: 12, fontWeight: 600, fontFamily: t.disp, textAlign: "center", padding: "8px 0" }}>
                            View full calendar →
                        </button>
                    )}
                </MobileAccordion>
